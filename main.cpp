...
ostream &operator<<(ostream &output, vector_complex &v) {
    output << "<Vector_complex cu " << v.nr_elem << " elemente: [" ;
    for (int i = 0; i < v.nr_elem; i++) {
        if ((int)v.p[i].imag()>0)
            output <<" " <<v.p[i].real() << " + " << v.p[i].imag() << "i,";
        else
            output <<" " << v.p[i].real() << " - " << (-1)*v.p[i].imag() << "i,";
    }
    output << "]>" <<'\n';
    return output;
}
...
void vector_complex::afisare_module() {
    cout << "Vectorul de module sortat:[ ";
    for (int i = 0; i < nr_elem; i++)
        cout << abs(p[i]) << ", ";
    cout << "]";
    cout << '\n';
